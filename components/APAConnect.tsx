...
export const APAConnect: React.FC<Props> = ({
  ...
  const inputStyle: React.CSSProperties = {
    background: 'rgba(57,167,201,0.04)', border: '1px solid rgba(57,167,201,0.2)',
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Globe className="w-6 h-6" style={{ color: '#39A7C9' }} />
        <div>
      ...
      {!connection ? (
        ...
        <form onSubmit={handleConnect} className="space-y-4 max-w-md p-5 rounded-lg"
          style={{ background: 'rgba(9,30,22,0.6)', border: '1px solid rgba(57,167,201,0.12)' }}>
          <div>
          ...
          <button type="submit" disabled={busy}
            ...
            style={{ background: busy ? 'rgba(57,167,201,0.15)' : '#39A7C9', color: busy ? '#39A7C9' : '#0A1F17',
          ...
          <div className="flex items-start gap-2 text-[11px] pt-1" style={{ color: 'rgba(239,231,214,0.45)' }}>
            <ShieldCheck className="w-3.5 h-3.5 mt-0.5 shrink-0" style={{ color: 'rgba(57,167,201,0.6)' }} />
        ...
        <div className="space-y-5">
          <div className="flex flex-wrap items-center justify-between gap-3 p-4 rounded-lg"
            style={{ background: 'rgba(57,196,107,0.05)', border: '1px solid rgba(57,196,107,0.2)' }}>
            <div className="flex items-center gap-2 text-sm">
              <CheckCircle2 className="w-4 h-4" style={{ color: '#39C46B' }} />
            ...
            <div className="flex items-center gap-2">
              <button onClick={refreshTeams} disabled={busy}
                ...
                style={{ background: 'rgba(57,167,201,0.08)', border: '1px solid rgba(57,167,201,0.25)', color: '#39A7C9' }}>
                <RefreshCw className={`w-3.5 h-3.5 ${busy ? 'animate-spin' : ''}`} /> Refresh
              </button>
              <button onClick={disconnect} disabled={busy}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded text-xs font-bold"
                style={{ background: 'rgba(209,74,60,0.08)', border: '1px solid rgba(209,74,60,0.25)', color: '#ff87b0' }}>
          ...
          <div>
            ...
            <select
              ...
              {connection.teams.map(t => (
                <option key={t.id} value={t.id} style={{ background: '#0A1F17' }}>
          ...
          <button onClick={loadRoster} disabled={busy || !connection.activeTeamId}
            ...
            style={{ background: '#39A7C9', color: '#0A1F17', fontFamily: 'Orbitron, sans-serif', letterSpacing: '.05em' }}>
          ...
          {preview && (
            <div className="rounded-lg overflow-hidden" style={{ border: '1px solid rgba(57,167,201,0.15)' }}>
              <div className="flex items-center justify-between px-4 py-3"
                style={{ background: 'rgba(57,167,201,0.05)' }}>
                ...
                <button onClick={importRoster} disabled={busy}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded text-xs font-bold"
                  style={{ background: '#39C46B', color: '#0A1F17' }}>
                ...
                <tbody>
                  {preview.players.map((p, i) => (
                    <tr key={i} style={{ borderTop: '1px solid rgba(57,167,201,0.06)' }}>
                      ...
                      <td className="px-4 py-2 font-mono" style={{ color: 'rgba(239,231,214,0.7)' }}>{p.matchesWon}/{p.matchesPlayed}</td>
                      <td className="px-4 py-2 font-mono" style={{ color: 'rgba(239,231,214,0.7)' }}>{p.ppm?.toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
